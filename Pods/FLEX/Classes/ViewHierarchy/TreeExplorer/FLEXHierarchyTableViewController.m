...
+ (instancetype)windows:(NSArray<UIWindow *> *)allWindows
    ...
    NSArray *allViews = [self allViewsInHierarchy:allWindows];
    NSMapTable *depths = [self hierarchyDepthsForViews:allViews];
    return [[self alloc] initWithViews:allViews viewsAtTap:viewsAtTap selectedView:selected depths:depths seletedVC:selectedVC];
}
...
- (instancetype)initWithViews:(NSArray<UIView *> *)allViews
                   viewsAtTap:(NSArray<UIView *> *)viewsAtTap
                 selectedView:(UIView *)selectedView
                       depths:(NSMapTable<UIView *, NSNumber *> *)depthsForViews seletedVC: (UIViewController *)seletedVC{
    self = [self initWithViews:allViews viewsAtTap:viewsAtTap selectedView:selectedView depths:depthsForViews];
    self.title = [NSString stringWithFormat:@"%@", [self findName:seletedVC]];
    return self;
}

- (NSString *)lookView:(id)instance in:(id)target {
...
- (NSString *)findName: (UIViewController *)seletedVC {
    UIViewController *currentVC = seletedVC;
    NSString *name = nil;
    for (UIView *view in [[self.viewsAtTap reverseObjectEnumerator] allObjects]) {
        if (view == currentVC.view) {
            name = [self lookView:self.selectedView in:currentVC];
        } else {
            if (view == self.selectedView) {
                continue;
            }
            name = [self lookView:self.selectedView in:view];
        }
